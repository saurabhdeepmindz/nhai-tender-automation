...
export class VectorizationController {
  ...
  async vectorizeQuery(@Param('queryId') queryId: string) {
    try {
      ...
      // Import job service to use manual trigger
      const { QueryVectorizationJob } = await import(
        '../jobs/query-vectorization.job'
      );
      const jobService = new QueryVectorizationJob(
        null, // Will be injected properly
        this.vectorizationService,
        null,
        null,
      );

      await jobService.vectorizeQueryById(queryId);
