...
try {
    $bulkUpdateResult = Invoke-RestMethod `
        -Uri "$baseUrl/bulk-update" `
        -Method Post `
...
try {
    ...
    Get-Content $csvFilePath -TotalCount 5 | ForEach-Object { Write-Host $_ -ForegroundColor Gray }
    Write-Host ""
} catch {
    Write-Host "✗ Failed!" -ForegroundColor Red
    Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Red
...
try {
    $exportXlsxResult = Invoke-RestMethod `
        -Uri "$baseUrl/export" `
        -Method Post `
        -ContentType "application/json" `
        -Body $exportXlsxBody `
...
try {
    $invalidResult = Invoke-RestMethod `
        -Uri "$baseUrl/bulk-update" `
        -Method Post `
        -ContentType "application/json" `
        -Body $invalidBody `
